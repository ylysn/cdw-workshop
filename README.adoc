...
Version : 1.0.0 `27th March 2023` https://github.com/DashDipti/cdw-workshop[Based on CDW Workshop by DashDipti] +
Version : 1.1.0 `7th February 2025` +
...
== Important URLs
1. *HOL Credentials* - Participants must fill their details here to get associated credentials and resources for the workshop. +
2. https://login.cdpworkshops.cloudera.com/auth/realms/field-marketing-emea/protocol/saml/clients/cdp-sso[Workshop login] - Open this in `incognitio` mode and authenticate yourself to enter CDP. +
...
Please use the login URL: https://login.cdpworkshops.cloudera.com/auth/realms/field-marketing-emea/protocol/saml/clients/cdp-sso[Workshop login]. +
Enter the `Workshop Login Username` and `Workshop Login Password` shared by your instructor. +

(*Note*: Note that your username would be something like `user001`). +
...
. Click on your `user name (Ex: User1`) at the top righ corner.
...
=== 3.1 Open Hue for CDW Virtual Warehouse - `demo-hive` +

- Click on the image:images/step3/hue.png[] button on the right upper corner of `demo-hive` as shown in the screenshot below. +
image:images/step3/31-1.png[] +



- Create new databases.
Enter the following query and then make sure that you enter the user assigned to you. In the screenshot the user is `user001`.
...
=== 3.2 Run the following DDL in HUE for the CDW Virtual Warehouse - `demo-hive` +
...
STORED AS TEXTFILE LOCATION '/airlines-raw/airlines-csv/flights' tblproperties("skip.header.line.count"="1");
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' 
STORED AS TEXTFILE LOCATION '/airlines-raw/airlines-csv/planes' tblproperties("skip.header.line.count"="1");
...
STORED AS TEXTFILE LOCATION '/airlines-raw/airlines-csv/airlines' tblproperties("skip.header.line.count"="1");
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' 
STORED AS TEXTFILE LOCATION '/airlines-raw/airlines-csv/airports' tblproperties("skip.header.line.count"="1");
----
...
=== 3.3 Run the following DDL in HUE for the CDW Virtual Warehouse - `demo-impala`. +

- Go to the page where now you will access HUE of an Impala virtual warehouse. Click on `HUE` for *`demo-impala`* as shown in the screenshot below.
...
- Go back to the CDP Console and observe the Impala Virtual Warehouse `demo-impala`. +
...
- Make sure that you are using the HUE of `demo-hive`. Else, click on `HUE` and go to the HUE browser. +
...
(a) Location: `s3a://.../warehouse/tablespace/external/hive/user001_airlines.db/planes` +
...
From the cloudera console click on -  `demo-impala`. +
...
- Open HUE for the CDW `Hive` Virtual Warehouse - `demo-hive`
...
- Open HUE for the CDW `Impala` Virtual Warehouse - `demo-impala`. +
image:images/step3/33-1.png[] +
...
- Get the details of the `snapshots` and download them as CSV File. +
...
- From the CSV File just copy the first value of the timestamp. It could be the date or the timestamp. Paste it in the `create_ts` box. In our case the value was `2025-02-07 13:39:52.337000000`. Then execute the higlighted query only (*1st query*).
image:images/step5/53-3.png[] +

- From the notepad just copy the second value of the snapshot id. In our case the value was `2325326240842793966`. Paste it in the `snapshot_id` box. Then execute the higlighted query only (*2nd query*).
...
- This can be used for both Iceberg tables and Hive Tables to improve performance. Go to the Cloudera console and look for `demo-hive`. Click on the `Hue` button on the right upper corner of `demo-hive` as shown in the screenshot below. +
...
- Change the radio button to select appropriate data lake. In this case it is `ya01-aw-dl`. +
image:images/step6/6-3.png[] + 

- Filter for Assets we created - below the Data Lakes on the left of the screen under Filters, select `TYPE` to be `Iceberg Table`.  The right side of the screen will update to reflect this selection. +
...
`Policy Name`: <user_id>_RowLevelFilter (Ex: user001_RowLevelFilter) +
`Hive Database`: <user_id>_airlines (Ex: user001_airlines) +
...
- Test the policy is working - Open `HUE` for the CDW Impala Virtual Warehouse - `demo-impala` and execute the following query. +
...
- Then click on `Data Visualization` option in the left windowpane. You'll see an option `Data VIZ` next to the data-viz application with the name `demo-dataviz`. It should open a new window.+
...
- Create a connection - click on the NEW CONNECTION button on the left menu. Enter the details as shown in the screenshot and click on `TEST`.  +
    ...
    Connection name: `<user_id>-airlines-lakehouse` (Ex-`user001-airlines-lakehouse`). +
...
- Select the appropriate `Database Name` base on your user id (Ex: `user001_airlines`) and table name `planes`.  
...
- Now we will create join between another table. Click on `+` icon next to `flights` as shown below. Select the appropriate `Database Name` based on your <user_id> (Ex: `user001_airlines`) and table name `airlines`. +
...
- Click on `+` icon next to `flights` as shown below. Select the appropriate `Database Name` based on your <user_id> (Ex: `user001_airlines`) and table name `airports`. +
